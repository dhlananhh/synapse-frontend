...
export const userService = {
  ...
  getMe: (): Promise<UserProfile> => {
    return userApiClient.get(`/me`).then(res => res.data);
  },

  getUserProfile: (userId: string): Promise<UserProfile> => {
    return userApiClient.get(`/${userId}`).then(res => res.data);
  },

  updateUserProfile: (userId: string, payload: UpdateUserProfilePayload): Promise<UserProfile> => {
    return userApiClient.patch(`/${userId}`, payload).then(res => res.data);
  },
  ...
  followUser: (userId: string): Promise<FollowResponse> => {
    return userApiClient.post(`/${userId}/follow`).then(res => res.data);
  },

  unfollowUser: (userId: string): Promise<void> => {
    return userApiClient.delete(`/${userId}/follow`).then(res => res.data);
  },

  acceptFollowRequest: (followerId: string): Promise<void> => {
    return userApiClient.patch(`/${followerId}/follow/accept`).then(res => res.data);
  },

  rejectFollowRequest: (followerId: string): Promise<void> => {
    return userApiClient.patch(`/${followerId}/follow/reject`).then(res => res.data);
  },

  cancelFollowRequest: (followingId: string): Promise<void> => {
    return userApiClient.delete(`/${followingId}/follow/cancel`).then(res => res.data);
  },
  ...
  getFollowers: (userId: string, page = 1, limit = 20): Promise<FollowerResponse[]> => {
    return userApiClient.get(`/${userId}/followers`, { params: { page, limit } }).then(res => res.data);
  },

  getFollowing: (userId: string, page = 1, limit = 20): Promise<FollowingResponse[]> => {
    return userApiClient.get(`/${userId}/following`, { params: { page, limit } }).then(res => res.data);
  },
