  ...
  return (
    <div className="flex items-center justify-between p-3 rounded-lg border hover:bg-muted/50 transition-colors">
      ...
      >
        ...
        <div className="flex flex-col">
          <span className="font-semibold text-sm group-hover:underline flex items-center gap-1">
            { member.username }
            { member.role === "OWNER" && <Crown className="w-3.5 h-3.5 text-yellow-500 fill-yellow-500" /> }
            { member.role === "MODERATOR" && <Shield className="w-3.5 h-3.5 text-blue-500 fill-blue-500" /> }
          </span>
      ...
      <div>
        ...
        { member.role === "MODERATOR" && <Badge variant="secondary" className="text-xs">Mod</Badge> }
      </div>
    </div>
  );
}
...
export default function CommunityMembersPage({ params }: PageProps) {
  ...
  }, [ community?.id, debouncedSearchTerm, nextCursor ]);
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Total { community.memberCount.toLocaleString() } members
              </p>
            </div>
