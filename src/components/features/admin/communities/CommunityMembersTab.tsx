...
export default function CommunityMembersTab({ communityId, limit = 20 }: Props) {
  ...
  const roleIndicator = (role?: string) => {
    switch ((role || '').toUpperCase()) {
      case 'OWNER':
        return <span title='Owner'><Crown className='w-4 h-4 text-yellow-500' /></span>
      case 'MODERATOR':
        return <span title='Moderator'><Shield className='w-4 h-4 text-indigo-500' /></span>
      default:
        return null
    }
  }
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='border rounded-md overflow-hidden bg-card'>
        ...
        <div
          id='members-list'
          className={ `px-3 pb-3 transition-[max-height] duration-200 ease-in-out overflow-hidden ${membersOpen ? 'pt-2 max-h-[1000px]' : 'max-h-0'
            }` }
        >
