...
export function CommunityCreationWizard() {
  ...
  const handleInfoSubmit = async (data: TCreateCommunitySchema) => {
    ...
    try {
      ...
      toast.error('Failed to create community', {
        description:
          error.response?.data?.errors[0]?.message || 'Please check the name and try again.',
      })
    } finally {
  ...
  return (
    <div>
      {step === 'info' && (
        <CommunityInfoForm onSubmit={handleInfoSubmit} isSubmitting={isSubmittingInfo} />
      )}
      {step === 'images' && createdCommunity && (
        <CommunityImageForm community={createdCommunity} onFinish={handleFinish} />
      )}
    </div>
  )
}
