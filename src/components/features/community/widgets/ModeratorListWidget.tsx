  ...
  return (
    <li>
      <Link
        ...
        className='flex items-center gap-3 group rounded-md transition-colors px-2 py-1 hover:bg-muted/60'
      >
  ...
  useEffect(() => {
    ...
    const loadModerators = async () => {
      ...
      try {
        ...
        const res = await communityService.getMembers(communityId, { role: 'MODERATOR', limit: 5 })
        ...
        const profiles = await userService.getSimpleProfiles(userIds)
        if (!mounted) return
