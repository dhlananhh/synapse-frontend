...
import { CommunityFlairForm } from "../forms/CommunityFlairForm";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function ManageFlairsDialog({ community }: ManageFlairsDialogProps) {
  ...
  const handleDelete = async (flairId: string) => {
    try {
      ...
      toast.success("Flair deleted successfully!");
    } catch (error) {
