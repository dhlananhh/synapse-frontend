...
import { Community, CommunityRule } from '@/types/services/community'
...
export function ManageRulesDialog({ community, rules, setRules, trigger }: ManageRulesDialogProps) {
  ...
  const handleRuleUpdated = (updatedRule: CommunityRule) => {
    setRules((prev) =>
      prev.map((r) => (r.id === updatedRule.id ? updatedRule : r)).sort((a, b) => a.order - b.order)
    )
    setEditingRule(null)
  }
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        ...
        <DialogContent className='max-w-2xl'>
          <DialogHeader>
            ...
            <DialogDescription>Set clear expectations for your community.</DialogDescription>
          </DialogHeader>

          <div className='py-4 space-y-4'>
            <h3 className='text-sm font-semibold text-muted-foreground'>CREATE A NEW RULE</h3>
            ...
            <div className='space-y-2'>
              <h3 className='text-sm font-semibold text-muted-foreground'>EXISTING RULES</h3>

              {isLoading ? (
                <Loader2 className='animate-spin' />
              ) : rules.length === 0 ? (
                <p className='text-center text-sm py-4'>No rules created yet.</p>
              ) : (
                <ul className='space-y-3'>
                  {rules
                    ...
                    .map((rule, index) => (
                      <li key={rule.id} className='p-4 rounded-lg border'>
