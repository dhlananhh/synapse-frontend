...
export const fetchPostById = async (
  ...
  if (postId.includes("error")) {
    throw new Error(
      "Failed to fetch post from the database. The connection timed out."
    );
  }
...
export const deletePost = async (
  ...
  if (postIndex === -1) {
    console.warn(
      "Attempted to delete a post that was not found."
    );
    return;
  }
...
const findCommentAndParent = (
  ...
  for (const comment of comments) {
    ...
    if (comment.replies) {
      const found = findCommentAndParent(
        comment.replies,
        commentId
      );
...
export const updateComment = async (
  ...
  const { comment } = findCommentAndParent(
    post.comments,
    commentId
  );
  if (!comment)
...
export const deleteComment = async (
  ...
  const { parent, comment } = findCommentAndParent(
    post.comments,
    commentId
  );
  if (parent && comment) {
...
export const fetchCommunityMembers = async (
  ...
  const membersWithRoles = allMockUsers.map(
    (user, index) => ({
      ...user,
...
export const deleteCommunity = async (
  ...
  if (communityIndex === -1) {
    console.warn(
      "Attempted to delete a community that was not found."
    );
    throw new Error("Community not found.");
  }

  const postsToDelete = mockPosts.filter(
  ...
  console.log(
    `Community with ID ${communityId} and all its posts have been deleted.`
  );
};
...
export const updateCommunityModerators = async (
  ...
  if (!community) {
    throw new Error(
      "Community not found while updating moderators."
    );
  }

  community.moderatorIds = moderatorIds;
  console.log(
    `Moderators for community ${community.slug} updated.`
  );
};
...
export const fetchUsersByIds = async (
  ...
  await new Promise((resolve) => setTimeout(resolve, 750));
  const foundUsers = allMockUsers.filter((user) =>
...
export const createCommunity = async (
  ...
  const slugExists = mockCommunities.some(
    (c) => c.slug === data.slug
  );
  if (slugExists) {
    throw new Error(
      "A community with this URL (slug) already exists."
    );
  }
...
export const uploadImage = async (
  ...
  console.log(
    `Simulating upload for file: ${file.name} (${(file.size / 1024).toFixed(2)} KB)`
  );
  await new Promise((resolve) => setTimeout(resolve, 1500));
...
export const fetchCommunityModerators = async (
  communityId: string
): Promise<CommunityMemberWithRole[]> => {
  ...
  const owner = allMockUsers.find(
    (u) => u.id === community.ownerId
  );
  const moderators = allMockUsers.filter((u) =>
    community.moderatorIds.includes(u.id)
  );
...
export const fetchFlairsForCommunity = async (
  communityId: string
): Promise<Flair[]> => {
  await new Promise((r) => setTimeout(r, 400));
  return mockFlairs.filter(
...
export const updateFlair = async (
  flairId: string,
  data: TFlairSchema
): Promise<Flair> => {
  await new Promise((r) => setTimeout(r, 600));
  const index = mockFlairs.findIndex(
    (f) => f.id === flairId
  );
...
export const deleteFlair = async (
  flairId: string
): Promise<void> => {
  await new Promise((r) => setTimeout(r, 600));
...
export const giveAwardToPost = async (
  postId: string,
  award: Award
): Promise<void> => {
  await new Promise((resolve) => setTimeout(resolve, 1000));
...
export const fetchPosts = async (
  ...
  if (options.filterByCommunityId) {
    visiblePosts = visiblePosts.filter(
      (p) => p.community.id === options.filterByCommunityId
    );
  }
  if (options.filterByFlairId) {
    visiblePosts = visiblePosts.filter(
      (p) => p.flair?.id === options.filterByFlairId
    );
  }
